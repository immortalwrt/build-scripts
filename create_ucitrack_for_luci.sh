...
function auto_create_ucitrack() {
	luci_app_list="$(find applications -maxdepth 1 | grep -Eo "applications/luci-app-[a-zA-Z0-9_-]+" | sort -s)"

	[ "$(echo -e "$luci_app_list" | wc -l)" -gt "0" ] && for i in $luci_app_list
	do
		if check_if_ucitrack_exist "$i"; then
			__warning_bg "$i: has ucitrack file already, skipping..." | tee -a "create_ucitrack_for_luci.warn"
		elif check_config_files "$i"; then
			__error_bg "$i: has no/multi config file(s), skipping..." | tee -a "create_ucitrack_for_luci.err"
		elif check_init_files "$i"; then
			__error_bg "$i: has no/multi/procd init script(s), skipping..." | tee -a "create_ucitrack_for_luci.err"
		else
			create_ucitrack_file "$i/root/usr/share/ucitrack" "${i##*/}" "$(get_target_name "$i" "config")" "$(get_target_name "$i" "init.d")"
			__success_bg "$i: ucitrack file has been generated." | tee -a "create_ucitrack_for_luci.ok"
		fi
	done
}
